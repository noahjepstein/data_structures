...
//LOWEST UNITS PRIORITY
//dispatch determines which assembly line has the lowest number of 
//units currently in its queue. it then pushes a package from the
//prebuffer into that assembly line.
void Factory::dispatch()
{
    ...
    if (!prebuffer->isEmpty() && t >= prebuffer->getFront().arrivalTime) {
        ...
        for (int i = 0; i < nLines; i++){
            ...
            else if (aLine[i]->getTotalUnits() <= aLine[min]->getTotalUnits()){
                min = i; //new lowest 
            }                      
