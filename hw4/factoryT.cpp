...
//using dynamic array of ptrs to assembly lines. 
Factory::Factory(int n_lines, double* rates)
{
    ...
    for (int i = 0; i < nLines; i++){
        ...
        //gives each assembly line a rate and the line number
        //indexing for lines starts at 1, not 0
        aLine[i] = new AssemblyLine(rates[i], i+1);
    }
...
//LOWEST TIME PRIORITY
//dispatch determines which assembly line will take the least amount of time
//to process by taking the total units on the line by the rate of the line. 
//it then pushes a package from the prebuffer into that assembly line.
void Factory::dispatch()
{
    ...
    if (!prebuffer->isEmpty() && t >= prebuffer->getFront().arrivalTime) {
        ...
        //find appropriate line. (the one with the fewest packages)
        //stillWorking conditional is to keep program from changing min
        //when no packages are on the line (since all lines are equally 
        //valued when they have zero packages on them)
        for (int i = 0; i < nLines; i++){
            
            if (!stillWorking()) {
                min = 0;
            }
            //if you find a new fastest "time prediction" then it's the min         
            else if ( (aLine[i]->getTotalUnits() / aLine[i]->getRate())
               <= (aLine[min]->getTotalUnits() / aLine[min]->getRate()) ){
                
                min = i; //new lowest 
            }
        }            

        //pops package from prebuffer and puts it into the right aLine.
        aLine[min]->process(prebuffer->pop()); 
    }    
